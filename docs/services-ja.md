...
```
src/services/<name>/
  index.ts      # 公開 API — service.ts と types.ts のみを再エクスポート
  service.ts    # サービス実装とエクスポートされるシングルトン
  interface.ts  # サービスインターフェース（契約）
  types.ts      # リクエスト/レスポンス型・ドメイン型
  helper.ts     # 副作用のない純粋なユーティリティ関数 — 内部専用
  converter.ts  # Chrome API の型からドメイン型への変換 — 内部専用
  internal.ts   # エラークラスとロガーインスタンス — 内部専用
  container.ts  # 依存性注入（クロスサービスの依存が存在する場合のみ） — 内部専用
```

## 公開 API の制約

**サービス外からインポートできるのは `index.ts` に記載されたファイルのみです。**

`index.ts` は `service.ts` と `types.ts` のみを再エクスポートします。
...
外部コードはサービスのインデックス経由でのみインポートしなければなりません。
...
`helper.ts`・`converter.ts`・`internal.ts`・`container.ts` はサービスの実装詳細であり、そのサービスディレクトリの外部からアクセスしてはいけません。

### この制約がある理由

各サービスは `index.ts` を通じて意図的な公開インターフェースを提供しています。インポートをその範囲に限定することで、以下の3つの保証が得られます。

- **リファクタリングの自由** — `helper.ts`・`converter.ts` などの内部ファイルは、サービス外に影響を与えることなく自由にリネーム・分割・削除できます。外部コードが直接インポートできる状態では、リファクタリングのたびに呼び出し元が壊れるリスクが生じます。
- **カプセル化** — `internal.ts` にはエラークラスとロガーが含まれています。これらを外部に公開すると、サービスの内部実装が漏れ出し、サービス間に暗黙の結合が生まれます（例: `tab` サービスの外で `TabServiceError` をキャッチするなど）。
- **契約の単一の情報源** — `index.ts` は、そのサービスが安定した API として提供するものを明示的に宣言する場所です。そこに記載されていないものは、意図的に不安定（実装詳細）として扱われます。

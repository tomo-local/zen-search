...
const StatefulAccordion = ({ defaultOpen = false, ...props }) => {
  const [isOpen, setIsOpen] = useState(defaultOpen);
  const handleToggle = () => {
    setIsOpen(!isOpen);
    action("Toggled")(isOpen ? "closed" : "opened");
  };
  return <Accordion isOpen={isOpen} onToggle={handleToggle} {...props} />;
};

export const Default: Story = {
  render: () => (
    <StatefulAccordion className="border border-b border-gray-300 rounded-lg">
